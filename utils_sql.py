...
# tricks for dropping inexecutable code
bad_commands = {
    'SELECT first_name , country_code , birth_date FROM players, matches, rankings',
    'SELECT Model FROM model_list WHERE ModelId IN (SELECT ModelId FROM car_names WHERE MakeId IN (SELECT MakeId FROM car_names WHERE Model IN (SELECT Model FROM car_names WHERE Make IN (SELECT Make FROM car_names WHERE ModelId IN (SELECT ModelId FROM cars_data WHERE MPG IN (SELECT MAX(MPG) FROM cars_data))))));',
    'SELECT winner_name ,  winner_rank_points FROM matches ,  rankings ,  players ORDER BY winner_rank_points DESC LIMIT 1',
    'SELECT winner_name ,  winner_rank FROM matches JOIN players JOIN rankings ORDER BY winner_age ASC LIMIT 3',
    'SELECT winner_name FROM matches WHERE matches.winner_id  IN  (SELECT player_id FROM rankings WHERE rankings.ranking_date  IN  (SELECT ranking_date FROM rankings WHERE year  =  2013) AND rankings.ranking_date  IN  (SELECT ranking_date FROM rankings WHERE year  =  2016))',
    'SELECT first_name ,  country_code ,  birth_date FROM players LEFT JOIN matches ON players.player_id  =  winner_id LEFT JOIN rankings ON players.player_id  =  winner_id WHERE winner_rank_points = (SELECT MAX(winner_rank_points) FROM players LEFT JOIN matches ON players.player_id  =  winner_id LEFT JOIN rankings ON players.player_id  =  winner_id);',
...
def execute(db, p_str, pred, timeout):
    ...
    try:
        ...
        assert p_str.strip() not in bad_commands
